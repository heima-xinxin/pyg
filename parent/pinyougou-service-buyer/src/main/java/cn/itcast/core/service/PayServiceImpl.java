...
//           param.put("total_fee",String.valueOf(payLog.getTotalFee()));//这是需要支付的钱
           param.put("total_fee","1");//这是需要支付的钱 设置为1分
//        终端IP 	spbill_create_ip 	是 	String(16) 	123.12.12.123 	APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP。
           param.put("spbill_create_ip","127.0.0.1");//因为局域网 所以响应不到 所以这个参数没有意义
...
//        通知地址 	notify_url 	是 	String(256) 	http://www.weixin.qq.com/wxpay/pay.php 	异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数。
           param.put("notify_url","www.baidu.com");//无意义 相应不到
...
//        说明详见参数规定
        ...
        client.setXmlParam(xml);//发送的xml数据
